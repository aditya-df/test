      ...
      <CardHeader className="border-b rounded-t-lg bg-gray-50 dark:bg-zinc-950">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <CardDescription>
              Connect your app with trusted third-party tools and services in
              one place.
            </CardDescription>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
          ...
          <div className="flex items-center gap-2">
            <Badge variant="outline" className="text-xs">
              {filteredIntegrations?.length || 0} Integration
            </Badge>
          </div>
        </div>

        {loading && !filteredIntegrations?.length ? (
          <TableSkeleton columnDefs={columnDefs} />
        ) : (
          <div ref={tableContainerRef}>
            <TableComponent
              loading={loading}
              rowData={filteredIntegrations}
              columnDefs={columnDefs}
              key={JSON.stringify(filteredIntegrations)}
            />
          </div>
        )}
